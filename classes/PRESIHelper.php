...
private static $banners = null;
  ...
  static public function tcButtonShortcodes(){
      ...
      $results = $wpdb->get_results( "SELECT * FROM ".PRESI_TABLE_SCHEDULES , ARRAY_A );
      ...
      for($i=0; $i<count($results); $i++){
          ...
          $shortcode->shortcode = PRESIHelper::shortcodeWithPID($results[$i]["id"]);
  ...
  static public function getPortfolioWithId($pid){
      ...
      if(count($res)) {
          ...
          $query = $wpdb->prepare("SELECT * FROM " . PRESI_TABLE_PROJECTS . " WHERE pid = %d", $pid);
          $res = $wpdb->get_results($query, OBJECT);
          ...
          if($schedule->options && !empty($schedule->options)){
              ...
              $schedule->options = json_decode( base64_decode(PRESIHelper::getScheduleDefaultOptions(0, $gridType)), true);
          }
          if(!empty($schedule->extoptions)){
              $schedule->extoptions = json_decode($schedule->extoptions, true);
          } else {
              $schedule->extoptions = array('type' => PRESIGridType::ALBUM);
          }
      }
  ...
  static public function getPortfolios($pid){
    ...
    global $wpdb;
    $presidium = null;
    $query = "SELECT ".PRESI_TABLE_PORTFOLIOS.".*, ".PRESI_TABLE_POSITIONS.".name FROM ".
    PRESI_TABLE_PORTFOLIOS." INNER JOIN ".PRESI_TABLE_POSITIONS." ON ".PRESI_TABLE_PORTFOLIOS.".".
    PRESI_TABLE_POSITIONS_ID." = ".PRESI_TABLE_POSITIONS.".".PRESI_TABLE_POSITIONS_ID;

    $presidium = $wpdb->get_results( $query , OBJECT );

    if( count($presidium) ){
        foreach( $presidium as $portfolio ){
  ...
  {
      if (is_null(PRESIHelper::$banners)) {
          $lastLoadedAt = get_option(PRESI_BANNERS_LAST_LOADED_AT);
          $lastLoadedAt = !empty($lastLoadedAt) ? $lastLoadedAt : 0;
          if ($lastLoadedAt == 0 || time() - $lastLoadedAt > 1 * 24 * 60 * 60) {
              $result = PRESIHelper::sendApiRequest(array(
                  'pkg' => PRESI_LICENSE_TYPE,
                  'action' => 'get_banners'
              ), 'GET');
              $cache = array();
              if (!empty($result) && $result['rc'] == PRESIHelper::API_RC_OK) {
                  $cache = !empty($result['banners']) ? $result['banners'] : array();
                  update_option(PRESI_BANNERS_CONTENT, $cache);
                  update_option(PRESI_BANNERS_LAST_LOADED_AT, time());
              }
          } else {
              $cache = get_option(PRESI_BANNERS_CONTENT);
          }
          PRESIHelper::$banners = $cache;
      }
      if (!empty(PRESIHelper::$banners[$zone])) {
          return PRESIHelper::$banners[$zone];
      }
      return array();
  }
  ...
  {
      $result = wp_remote_post( PRESI_API_URL,
          array(
              'method' => $method,
              'timeout' => 45,
              'blocking' => true,
